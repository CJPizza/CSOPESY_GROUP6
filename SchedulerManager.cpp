...
void SchedulerManager::generateProcesses()
{
    ...
    while(isGenerating)
    {
        ...
        if(cpu_cycle_count % batch_process_freq == 0)
        { //activates for every batch_freq
        ...
        std::string pname = procname.str();
        
        proclines = distrib(gen);
        
        //create new process
        std::shared_ptr<Process> newProcess = std::make_shared<Process>(pname, proclines);

        fcfs_scheduler->addProcess(*newProcess, 0);
