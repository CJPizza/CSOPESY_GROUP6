...
void FCFSScheduler::printProgress()
{
    ...
    for (auto& queue : this->process_queues) {
        for (auto& process : queue) {
            ...
            std::cout << process.getProcessName() << "\t" << "(" << process.getTimeToStr() << ")" << "\t" << "Core: " << "N/A " << "\t" << process.getRemainingInstructions() << "/" << process.getTotalInstruction() << std::endl;
            } else {
            std::cout << process.getProcessName() << "\t" << "(" << process.getTimeToStr() << ")" << "\t" << "Core: " << process.getCpuID() << "  " << process.getRemainingInstructions() << "/" << process.getTotalInstruction() << std::endl;
            }
        }
    }
    /*
    ...
    for (auto& process : this->done_processes) {
        std::cout << process.getProcessName() << "\t" << "(" << process.getTimeToStr() << ")" << "\t" << "Finished" << "\t" << process.getTotalInstruction() - process.getRemainingInstructions() << "/"<< process.getTotalInstruction() << "\n";
    }
}
